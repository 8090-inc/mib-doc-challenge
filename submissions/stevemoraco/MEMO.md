...
## Measured performance — score provenance

All numbers are from the same blind 200-case holdout (20% stratified split, fixed seed, scored with the challenge's own `evaluate.py`; the holdout was excluded from rule fitting and threshold tuning). One number is the submissible execution path; the others are internal references, labeled as such.

- **Submissible path (the linked container, end-to-end via the Docker entrypoint): 113.55 / 150** — extraction 39.49/50, classification 59.25/80, calibration 14.80/20, **0 catastrophic false approvals**. This run used the container exactly as linked (post-repair — see below), on a macOS host; a 25-case `--network none` parity check against a Linux container run showed every differing case in the Linux container's favor (it recovers strictly more evidence), so we believe 113.55 is a floor for the containerized pipeline in the challenge environment.
- Internal rule-engine reference (non-submissible): **120.69 / 150** on a historical cached extraction — used during development to isolate rule-engine behavior from extraction. The gap between it and the container number is extraction quality on degraded scans, and is where our remaining effort goes.
- Repair note: during final verification we isolated and repaired an extraction-trust regression — a pixel-based hidden-text classifier was discarding legitimate field values on image-backed pages (measured −13.5 on a fixed 40-case slice; repaired to within 1.1 of the pre-regression reference, 0 false approvals throughout). **The submitted predictions and the linked container both contain the repaired code**: predictions were regenerated from a full re-extraction of all 5,000 validation PDFs after the repair commits landed.
