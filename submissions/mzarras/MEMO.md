...
label-truth 162/162 on train; transit/fee/embargo/staleness deny paths measure 95–100% where they
...
The holdout gate confirmed this is the binding failure mode. Run **once**, at the pre-registered
checkpoint, the aggressive configuration scored **119.76 with 5 CFAs** in 200 cases — all five
consistent with the silent-denial pattern. The final decision: approvals additionally require
*positively-read clean evidence* — a decoded `Observed flags: none` B-13 line or an approving
note — which is precisely the document silent-denial packets structurally lack. The holdout was
used only to select between two pre-registered configurations (probability guard alone vs.
structural guard), never for fitting; the structural rule is the original design principle "never
approve on absence of evidence," selected, not tuned. Cost on tune: 127.95 → 127.02. Expected
...
| **Final (structural approval guard)** | **127.02** | **0** |
| Holdout, run once (aggressive config) | 119.76 | 5 |
...
(~2.1 points less). **Expect ~120–125 on unseen labeled data.** CFA is 0 by construction on the
structural path. Calibration (17.2/20, mean Brier 0.069) is near its mathematical ceiling — with
residual-pool accuracy around 58%, honest confidences cannot push Brier much lower.
...
|---|---|---|
| 6 revoked sponsors (3 named in manual + SPN-2718/9090/7331) | in-packet adjudicator revocation lines / signatures | fires only on in-packet evidence, post-correction sponsor, non-DIP-1 visa positively read |
