...
- **Official train: 128.69 / 150**, the full Docker scoring contract over all 1,000 cases.
...
- **0 fallback rows**; 343/344 recovered adjudicator notes adjudicate correctly.
- **5,000 / 5,000 validation rows**, written by the frozen image, validated against the manifest.
- **5.52 s/PDF against the 6.0 s budget, 0.36 GiB image** (quiet-box docker_check, 100 PDFs).
...
- **One misread note, MIB-000497.** A legible note reads "Finding: NEEDS_REVIEW"; damage truncates the line, and the fuzzy matcher mis-binds the fragment to DENIED under the image's Tesseract 5.3. It is the single disagreement behind 343/344, costs −0.08, and is priced into the scores above. Found post-freeze; the fix is built and validated, but staged behind the frozen image rather than slipped into it.
...
- **Ship the staged note-matcher fix** (the MIB-000497 truncation bug): built, 40 new tests, red-team re-passed, corpus-swept — staged behind the frozen image rather than slipped in post-freeze.
- **Close the reader's fit/serve gap** so the note-band re-read mints in production: it already recovers a known APPROVED case under the dev OCR; the image's older Tesseract drops it.
- **Constrained-menu field matching behind a confidence gate:** recovered a destroyed purpose field correct-at-margin in the lab; next is the corpus sweep and the prefix-fit gate that separates its wins from its hallucinations.
